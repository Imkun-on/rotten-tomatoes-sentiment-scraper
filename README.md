...
An interactive CLI tool that scrapes movie and TV show reviews from **Rotten Tomatoes**, performs **sentiment analysis** using a pre-trained transformer model, and exports results to **CSV**, **JSON**, or **Excel**.
...
1. **Choose search mode**:
   - `1` — Search by name (text query)
   - `2` — Paste a direct Rotten Tomatoes URL
...
4. **Scrape reviews**: the tool scrapes all critic (Tomatometer) and audience (Popcornmeter) reviews via RT's internal API, with a real-time progress bar

5. **Sentiment analysis** (optional): analyzes the text of each review and assigns a 1-5 star score

6. **Browse reviews**: paginated display in the terminal, showing both the original rating and the sentiment score
