...
- [Usage & Examples](#usage--examples)
  - [Example 1: Search by Query (Movie)](#example-1-search-by-query-movie)
  - [Example 2: Direct URL](#example-2-direct-url)
  - [Example 3: TV Series with Per-Season Scraping](#example-3-tv-series-with-per-season-scraping)
...
- **Real-time progress bars** for both scraping and sentiment analysis, with the exact total computed in advance from RT's `mediaScorecard` (no rolling estimate)
...
### Example 1: Search by Query (Movie)
...
### Example 3: TV Series with Per-Season Scraping

TV series on Rotten Tomatoes are **not** reviewed at the show level — every Tomatometer/Popcornmeter score belongs to a specific season, and the per-show review API returns 404. The CLI handles this transparently: when you select a TV title, you're shown a Seasons table, you pick which one(s) to scrape, and the tool pulls the reviews for each selected season and merges them into a single batch.
...
──────────────────── → Search on Rotten Tomatoes ────────────────────
  ...
  ├────┼──────────────────────────────┼──────┼──────────┤
  │ 1  │ The Boys                     │ 2019 │ 93       │
  │ 2  │ The Boys: Diabolical         │ 2022 │ 81       │
  │  …                                │      │          │
  └────┴──────────────────────────────┴──────┴──────────┘

  Select a result (number) (1): 1

  ✔ Selected: The Boys (2019)

  ╭─────────────── The Boys ───────────────╮
  │  • Title:         The Boys              │
  │  • Year:          2019                  │
  │  • Genre:         Action, Sci-Fi, …     │
  │  • Plot:          A group of vigilantes │
  │    sets out to take down corrupt        │
  │    superheroes…                         │
  │  • Tomatometer:   93%   (561 Reviews)   │
  │  • Popcornmeter:  74%   (10,000+ Ratings)│
  ╰─────────────────────────────────────────╯
```

The CLI detects the `/tv/` URL and fetches the season list (in parallel). The Seasons table shows **exact review counts** taken from each season's `mediaScorecard` so the progress bar can be sized correctly later:

```
                            Seasons (5)
  ╭──────┬──────────┬──────────┬─────────────┬──────────────┬───────╮
  │  #   │ Season   │ Episodes │ Tomatometer │ Popcornmeter │ Total │
  ├──────┼──────────┼──────────┼─────────────┼──────────────┼───────┤
  │  1   │ Season 1 │    8     │  85% (106)  │ 90% (1,437)  │ 1,543 │
  │  2   │ Season 2 │    8     │  97% (105)  │ 84% (  743)  │   848 │
  │  3   │ Season 3 │    8     │  98% (153)  │ 72% (  583)  │   736 │
  │  4   │ Season 4 │    8     │  92% (142)  │ 54% (1,875)  │ 2,017 │
  │  5   │ Season 5 │    8     │  95% ( 55)  │ 74% (  124)  │   179 │
  ╰──────┴──────────┴──────────┴─────────────┴──────────────┴───────╯

  Selection syntax: 1 | 1-3 | 1,3,5 | all
  Choose season(s) (all): 1-2
...
──────────────────── ⚙ Scraping reviews ────────────────────
  ⠋ Scraping  ████████████████████████████  100%  2391/2391  │ 0:03:24 → 0:00:00

  Run sentiment analysis? [y/n] (y): y
  …
```

The exported CSV/JSON/Excel contains reviews from every selected season, with the `title` column annotated as `"The Boys — Season 1"`, `"The Boys — Season 2"`, etc., so you can group by season later.
...
For TV series, the scraper iterates over the user-selected seasons and merges all the resulting reviews into one batch. The total size of the progress bar is computed up-front from each season's `mediaScorecard.criticsScore.reviewCount + audienceScore.reviewCount`, so the bar starts at the right number and the final count is reliable.
